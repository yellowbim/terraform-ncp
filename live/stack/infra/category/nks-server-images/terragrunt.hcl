locals {
  ...
  output_dir       = "${get_repo_root()}/projects/${local.selected_project}/_generated/${local.selected_env}/category/nks-server-images"
}
...
terraform {
  source = "${include.root.locals.root}/modules/category/nks-server-images"
...
inputs = {
  ...
  keyword    = local.env_cfg.locals.nks_server_images.keyword
}
