locals {
  ...
  output_dir       = "${get_repo_root()}/projects/${local.selected_project}/_generated/${local.selected_env}/category/nks-server-spec"
}
...
terraform {
  source = "${include.root.locals.root}/modules/category/nks-server-spec"
...
inputs = {
  software_code = local.env_cfg.locals.nks_server_spec.software_code
