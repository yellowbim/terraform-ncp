locals {
  ...
  output_dir       = "${get_repo_root()}/projects/${local.selected_project}/_generated/${local.selected_env}/category/server-specs"
}

# 이거 server_specs.json 파일은 정상적으로 만들긴 ₩
# 근데 이걸 사용해서 이미지랑 연동해서 써야하는데 일단은 미사용
include "root" {
...
terraform {
  source = "${include.root.locals.root}/modules/category/server-specs"
