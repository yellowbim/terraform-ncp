locals {
  ...
  output_dir       = "${get_repo_root()}/projects/${local.selected_project}/_generated/${local.selected_env}/category/nks-version"
}
...
terraform {
  source = "${include.root.locals.root}/modules/category/nks-version"
...
inputs = {
  ...
  keyword    = local.env_cfg.locals.nks_version.keyword
}
